...
export default function Home() {
  ...
  useEffect(() => {
    ...
    stations.forEach(s => {
      ...
      el.innerHTML = `
        <div style="width:28px;height:28px;border-radius:50%;background:${petrolColor};border:2px solid white;display:flex;align-items:center;justify-content:center">
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', height: '100vh', fontFamily: 'system-ui, sans-serif', fontSize: 14 }}>
        ...
        {/* SIDEBAR */}
        <div style={{ width: 340, display: 'flex', flexDirection: 'column', borderRight: '1px solid #e5e7eb', background: '#fff', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '12px 14px', borderBottom: '1px solid #e5e7eb' }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              ...
              <button onClick={locateMe} style={{ padding: '6px 11px', background: '#1d4ed8', color: 'white', border: 'none', borderRadius: 7, fontSize: 12, fontWeight: 500, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                Near me
              </button>
            </div>
          </div>
          ...
          <div style={{ overflowY: 'auto', flex: 1, padding: 8 }}>
            {filtered.length === 0 && (
              <div style={{ padding: 32, textAlign: 'center', color: '#9ca3af' }}>No stations match your filters.</div>
            )}
            {filtered.map(s => {
              ...
              return (
                <div key={s.id} onClick={() => { setSelected(s); map.current?.flyTo({ center: [s.lng, s.lat], zoom: 14 }) }}
                  style={{ padding: '10px 12px', borderRadius: 9, border: `1px solid ${isSelected ? '#1d4ed8' : '#e5e7eb'}`, background: isSelected ? '#eff6ff' : '#fff', marginBottom: 5, cursor: 'pointer', borderLeft: isCheapestPetrol ? '3px solid #16a34a' : isCheapestDiesel ? '3px solid #d97706' : '1px solid #e5e7eb' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                    <div>
                      <div style={{ fontWeight: 500, fontSize: 13 }}>{s.name} <span style={{ fontWeight: 400, color: '#6b7280' }}>· {s.town}</span></div>
                      <div style={{ fontSize: 11, color: '#9ca3af' }}>{s.county}</div>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2 }}>
                      ...
                      {isCheapestDiesel && <span style={{ fontSize: 10, background: '#eff6ff', color: '#1d4ed8', padding: '1px 6px', borderRadius: 4, fontWeight: 500 }}>Cheapest diesel</span>}
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: 6 }}>
                    {fuelFilter !== 'diesel' && (
                      <div style={{ padding: '5px 9px', borderRadius: 6, border: `1px solid ${isCheapestPetrol ? '#16a34a' : '#e5e7eb'}`, background: isCheapestPetrol ? '#f0fdf4' : '#f9fafb', minWidth: 60, textAlign: 'center' }}>
                        ...
                        <div style={{ fontSize: 9, color: STATUS_COLOR[s.petrolStatus?.status ?? 'unknown'] }}>{s.petrolStatus?.status ?? 'unknown'}</div>
                      </div>
                    )}
                    {fuelFilter !== 'petrol' && (
                      <div style={{ padding: '5px 9px', borderRadius: 6, border: `1px solid ${isCheapestDiesel ? '#d97706' : '#e5e7eb'}`, background: isCheapestDiesel ? '#fffbeb' : '#f9fafb', minWidth: 60, textAlign: 'center' }}>
                        ...
                        <div style={{ fontSize: 9, color: STATUS_COLOR[s.dieselStatus?.status ?? 'unknown'] }}>{s.dieselStatus?.status ?? 'unknown'}</div>
                      </div>
                    )}
                    <div style={{ flex: 1 }} />
                    <div style={{ textAlign: 'right', fontSize: 10, color: '#9ca3af' }}>
                      {dStr && <div>{dStr}</div>}
                      <div>{s.latestPrice?.source ?? ''}</div>
                    </div>
                  </div>
                </div>
              )
        ...
        <div style={{ flex: 1, position: 'relative' }}>
          ...
          {selected && (
            <div style={{ position: 'absolute', bottom: 0, right: 0, width: 290, background: '#fff', borderTop: '1px solid #e5e7eb', borderLeft: '1px solid #e5e7eb', borderRadius: '10px 0 0 0', padding: 16, zIndex: 10 }}>
              ...
              <div style={{ fontSize: 12, color: '#6b7280', fontFamily: 'monospace', marginBottom: 12 }}>{selected.brand} · {selected.county}</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
                <div style={{ padding: 10, borderRadius: 7, border: '1px solid #e5e7eb', background: '#f9fafb', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 10, color: STATUS_COLOR[selected.petrolStatus?.status ?? 'unknown'] }}>{selected.petrolStatus?.status ?? 'unknown'}</div>
                </div>
                <div style={{ padding: 10, borderRadius: 7, border: '1px solid #e5e7eb', background: '#f9fafb', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 10, color: STATUS_COLOR[selected.dieselStatus?.status ?? 'unknown'] }}>{selected.dieselStatus?.status ?? 'unknown'}</div>
                </div>
              </div>
